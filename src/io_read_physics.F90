...
!********** Added by Adolfo *********
!        INTEGER                                 :: coll_implicit_pair_num_sweep_tang
!*************************************
	...
	!----------------------------------------------------
        ...
        iline = 0
          ...
          ELSE IF (carg =='KSAI') THEN	  
             ...
             CALL physics_set_a(phys, a, stat_info_sub)
             CALL physics_set_b(phys, b, stat_info_sub)
             !***********************************
          ...
          ELSE IF (carg == 'BETA') THEN
             ...
             CALL physics_set_beta(phys,beta,stat_info_sub)
             !********************************************

           !**** Added by Adolfo for the integral fractional model ****
           ELSE IF (carg == 'TIME_SAVED') THEN
             
             !-----------------------------------------------
             ! Time saved (in units of tau(1)) for the integral fractional model.
             !-----------------------------------------------
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) time_saved
             CALL physics_set_time_saved(phys,time_saved,stat_info_sub)
             dt = physics_get_dt(phys, stat_info_sub)
           ...
           ELSE IF (carg == 'NPOINTS_INTEGRATION') THEN
             ...
             !-----------------------------------------------
             ! Number of points of integration to calculate the stress
             !  from the memory function and previous states
             !-----------------------------------------------
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) Npoints_integration

             !-- freq_integration is calculated --
             control = .TRUE.
             DO WHILE (control)
             ...
             CALL physics_set_Nsteps_memory(phys, Nsteps_memory, stat_info_sub)
             CALL physics_set_mem_function(phys, Nsteps_memory, E, tau, alpha, beta, dt, stat_info_sub)
             IF (stat_info_sub == -1) THEN
                stat_info = -1
                GOTO 9999
             ENDIF
             !************************************************************
...
!************ Added by Adolfo *********
!             coll_implicit_pair_num_sweep_tang  = 1
!**************************************
...
9999	CONTINUE
        ...
        IF ( stat_info == 0 ) THEN
		   ...
		   !*********** Added by Adolfo *********
!		   coll_implicit_pair_num_sweep_tang = coll_implicit_pair_num_sweep
!              CALL colloid_set_implicit_pair_num_sweep_tang(colloids, &
!                   coll_implicit_pair_num_sweep_tang,stat_info_sub)
		   !*************************************
              ...
              CALL colloid_set_implicit_pair_sweep_error(colloids, &
                   coll_implicit_pair_sweep_tolerance,stat_info_sub)
		   !********* Added by Adolfo ***********
!              CALL colloid_set_implicit_pair_sweep_error_tang(colloids, &
!                   coll_implicit_pair_sweep_tolerance,stat_info_sub)
		   !*************************************

...
!**************** Added by Adolfo **********
      SUBROUTINE physics_set_a(this,d_a,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_a
        INTEGER, INTENT(OUT)            :: stat_info
      ...
      END SUBROUTINE physics_set_a

      SUBROUTINE physics_set_b(this,d_b,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_b
        INTEGER, INTENT(OUT)            :: stat_info
...
!************ End of added by Adolfo *******
      ...
      SUBROUTINE physics_set_time_saved(this,d_time_saved,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_time_saved
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%time_saved =  d_time_saved
        
        RETURN
        
      END SUBROUTINE physics_set_time_saved
      !********************************************************
      ...
      SUBROUTINE physics_set_Nsteps_memory(this,d_Nsteps_memory,stat_info)
        ...
        TYPE(Physics), INTENT(INOUT)    :: this
        INTEGER, INTENT(IN)             :: d_Nsteps_memory
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%Nsteps_memory =  d_Nsteps_memory
        
        RETURN
        
      END SUBROUTINE physics_set_Nsteps_memory
      !********************************************************

      !**** Added by Adolfo for the integral fractional model ****
      SUBROUTINE physics_set_E(this,d_E,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_E
        INTEGER, INTENT(OUT)            :: stat_info
      ...
      !**** Added by Adolfo for the integral fractional model ****
      SUBROUTINE physics_set_beta(this,d_beta,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_beta
        INTEGER, INTENT(OUT)            :: stat_info
      ...
      END SUBROUTINE physics_set_beta
      !********************************************************
      ...
      SUBROUTINE physics_set_mem_function(this, Nsteps_memory, E, tau, alpha, beta, dt, stat_info)
        ...
        REAL(MK), INTENT(in)               :: E
        REAL(MK), INTENT(in)               :: tau
        REAL(MK), INTENT(in)               :: alpha
        REAL(MK), INTENT(in)               :: beta
        ...
        ALLOCATE(this%mem_function(Nsteps_memory))
        ...
        !    !-- Multimode
        !    DO J = 1, Nmodes
        !      total = total + (n_p(J) * kt_p / tau(J) * exp(- Delta_t / tau(J)))/Nmodes
        !    ENDDO
        ...
        !    !-- Single spring-pot
        !    IF (Delta_t .NE. 0) THEN
        !     this%mem_function(I) = -E *beta *Delta_t**(-beta-1.0_MK)
        !    ELSE
        !     this%mem_function(I) = 0.0_MK
        !    ENDIF
...
1000    CONTINUE
      ...
      END SUBROUTINE physics_set_mem_function
      !********************************************************

      !**** Added by Adolfo for the integral fractional model ****
      SUBROUTINE physics_set_Npoints_integration(this,d_Npoints_integration,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        INTEGER, INTENT(IN)            :: d_Npoints_integration
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%Npoints_integration =  d_Npoints_integration
        
        RETURN
        
      END SUBROUTINE physics_set_Npoints_integration
      !********************************************************

      !**** Added by Adolfo for the integral fractional model ****
      SUBROUTINE physics_set_freq_integration(this,d_freq_integration,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        INTEGER, INTENT(IN)             :: d_freq_integration
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%freq_integration =  d_freq_integration
        
        RETURN
        
      END SUBROUTINE physics_set_freq_integration
      !********************************************************

      !**** Added by Adolfo for the integral fractional model ****
      SUBROUTINE physics_set_steps_since_last_saved_pos(this,d_steps_since_last_saved_pos,stat_info)
        
        TYPE(Physics), INTENT(INOUT)    :: this
        INTEGER, INTENT(IN)             :: d_steps_since_last_saved_pos
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%steps_since_last_saved_pos =  d_steps_since_last_saved_pos
        
        RETURN
        
      END SUBROUTINE physics_set_steps_since_last_saved_pos
      !********************************************************
...
9999    CONTINUE
  ...
  !********** Added by Adolfo for the integral fractional model ************
  !---------------------------------------------------------
SUBROUTINE mittag_leffler(E, x, alpha, beta, kmin, kmax, tol, error_out)
  !---------------------------------------------------------
  ! The Mittag-Leffler function reads
  !    E_{alpha,beta}(x) = sum_{k=0}^\infinity x^k / Gamma(alpha k + beta),  alpha > 0, beta > 0
  ...
  k = 0
  DO WHILE(control)
     k_real = REAL(k, kind = MK)     
     E = E + x**k_real / gamma(alpha*REAL(k_real, KIND = MK) + beta)

     !-- The convergence is checked --
     IF (k > kmin) THEN
        IF ( ABS((E-E_old)/E) < tol) THEN
           control = .FALSE.
        ELSE
           k     = k + 1
           E_old = E
     ...
     IF (k > kmax) THEN
        error_out = 1
        WRITE(*,*) '*** Special function Mittlag-Leffler error: convergence '
        WRITE(*,*) '    was not achieved. ***'
        control = .FALSE.
        GOTO 1000 !-- End of subroutine --
     ENDIF
...
END SUBROUTINE mittag_leffler
!****************************************************

  !********** Added by Adolfo for the integral fractional model ************
  !---------------------------------------------------------
SUBROUTINE der_mittag_leffler(E, x, alpha, beta, kmin, kmax, tol, error_out)
  !---------------------------------------------------------
  ! This subroutine calculates the derivative of the Mittag-Leffler function.
  !---------------------------------------------------------
  IMPLICIT NONE
  REAL(MK), INTENT(out) :: E
  REAL(MK), INTENT(in)  :: x
  ...
  DO WHILE(control)
     k_real = REAL(k, kind = MK)
     E = E + k_real * x**(k_real-1) / gamma(alpha * k_real + beta)
     ...
     IF (k > kmax) THEN
        error_out = 1
        WRITE(*,*) '*** Special function der_Mittlag-Leffler error: convergence '
        WRITE(*,*) '    was not achieved. ***'
        control = .FALSE.
        GOTO 1000 !-- End of subroutine --
     ENDIF
...
END SUBROUTINE der_mittag_leffler

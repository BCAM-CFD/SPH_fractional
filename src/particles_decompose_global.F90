...
!!$           l_map_u,    l_map_au, stat_info)
      SUBROUTINE particles_decompose_global(this, &
           ...
           l_map_ct,   l_map_act,  &
           l_map_u,    l_map_au, &
           l_map_dx_prev, l_map_gradx_prev, &
           l_map_x_old, &
           stat_info)
!**********************************************************************
        !----------------------------------------------------
        ...
        LOGICAL, INTENT(IN), OPTIONAL   :: l_map_dx_prev
        LOGICAL, INTENT(IN), OPTIONAL   :: l_map_gradx_prev
        LOGICAL, INTENT(IN), OPTIONAL   :: l_map_x_old
        !***************************************************************
        ...
        END IF
        !*********** Added by Adolfo for the fractional model **********
        IF( PRESENT(l_map_x_old) ) THEN
           map_x_old = l_map_x_old
        END IF
        IF( PRESENT(l_map_dx_prev) ) THEN
           map_dx_prev = l_map_dx_prev       
        END IF
        IF( PRESENT(l_map_gradx_prev) ) THEN
           map_gradx_prev = l_map_gradx_prev       
        END IF
        !***************************************************************
	...
	!------------------------------------------
        ...
        IF( map_x_old) THEN
           ...
           CALL ppm_map_part(this%x_old,num_dim,&
                this%num_part_real,this%num_part_all,&
                topo_id,map_type,stat_info_sub)
        ...
        IF( map_dx_prev) THEN
           ...
           CALL ppm_map_part(this%dx_prev,num_dim*Npoints_integration,&
                this%num_part_real,this%num_part_all,&
                topo_id,map_type,stat_info_sub)
           
        END IF
        !----------------------
        ! Previous gradx tensors
        !----------------------
        
        IF( map_gradx_prev) THEN
           
           CALL ppm_map_part(this%gradx_prev,num_dim*num_dim*Npoints_integration,&
                this%num_part_real,this%num_part_all,&
                topo_id,map_type,stat_info_sub)
...
!!$        !********** Added by Adolfo ***********
!!$        ! This avoids some strange error when the noise is on
!!$        ! in the Bangor and Swansea hpcwales clusters.
!!$        WRITE(*,*) 'AAA ' ,stat_info, stat_info_sub
!!$        !***************************************

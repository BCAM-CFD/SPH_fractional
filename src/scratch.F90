...
!!$  	!----------------------------------------------------
!!$        
!!$        f_c = ( this%p(ip)/( this%rho(i)**2.0_MK) + this%p(jp)/(this%rho(j)**2.0_MK)) * gradw
!!$        
...
!!$!!!$             gradw / numi / numj /  dij
!!$        f_c =  visc_a * gradw / (this%rho(i) * this%rho(j) * dij)
!!$
!!$        
...
!!$!!!$             gradw * ev / numi / numj / dij
!!$        f_c = visc_b* gradw * ev / (this%rho(ip) * this%rho(jp) * dij)
!!$        
!!$        fip(1:num_dim) = fip(1:num_dim) + f_c * eij(1:num_dim) / this%m(ip)
!!$        fjp(1:num_dim) = fjp(1:num_dim) - f_c * eij(1:num_dim) / this%m(jp)
!!$        

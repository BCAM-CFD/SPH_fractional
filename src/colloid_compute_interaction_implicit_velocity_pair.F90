	...
	!********** Added by Adolfo ***********
        REAL(MK), DIMENSION(:,:), ALLOCATABLE   :: v_lub_normal
	!**************************************
	...
	!****************************************
        ...
        DO i = 1, num
           ...
           IF ( fa > mcf_machine_zero ) THEN
              ...
              !***** Commented by Adolfo to avoid too much output info *****
              ! IF ( dt_f < dt ) THEN
        ...
        !******** Added by Adolfo ******
	! Added when repulsion was included in the implicit scheme
	! It did not work fine, I do not know why
        ! Now the position is updated with the calculated velocity
!        DO i = 1, num 
!           this%x(1:dim,i) = this%x(1:dim,i) + this%v(1:dim,i,1) * dt
!        END DO
        !******* END of added by Adolfo *****

!!$	!*************** Added by Adolfo *******************
!!$	! Following the same structure than the normal lubrication
!!$	! force, the tangential force is added to the model
!!$	!****************************************************
!!$        !----------------------------------------------------
!!$        ! Do the pairwise sweep num_sweep times to update
!!$        ! velocities using only lubrcation correction forces.
...
!!$
!!$        !----------------------------------------------------
!!$        ! Back up velocity after normal lubrication
!!$	!  forces contributions.
!!$        !----------------------------------------------------
!!$        
!!$        ALLOCATE(v_lub_normal(dim,num))
!!$        v_lub_normal(1:dim,1:num) = this%v(1:dim,1:num,1)
!!$
!!$        
!!$        IF ( this%cc_lub_type > mcf_cc_lub_type_no ) THEN
!!$           
...
!!$           sweep_adaptive  = &
!!$                this%implicit_pair_sweep_adaptive
!!$           num_sweep_tang = this%implicit_pair_num_sweep_tang
!!$           
!!$           !-------------------------------------------------
!!$           ! Compute implicit velocity pairwise  interaction
!!$           ! num_sweep times.
!!$           !-------------------------------------------------
!!$           
!!$	   !** Next subroutine is in colloid_compute_interaction_implicit_velocity_pair_sweep.F90 **
!!$           CALL colloid_compute_interaction_implicit_velocity_pair_sweep_tang(&
!!$                this, dt, num_sweep_tang, stat_info_sub)
!!$           
!!$           IF ( stat_info_sub /= 0 ) THEN
!!$              PRINT *, __FILE__, __LINE__, &
!!$                   "velocity pair sweeps tang failed!"
!!$              stat_info = -1
!!$              GOTO 9999
!!$           END IF
!!$           
...
!!$
!!$              !----------------------------------------------
!!$              ! Back up velocity from previous num_sweep_tang sweeps.
!!$              !----------------------------------------------
!!$
!!$!-------- This array was already allocated in the normal part --              
!!$!              ALLOCATE(v_update(dim,num))
!!$              v_update(1:dim,1:num) = this%v(1:dim,1:num,1)
!!$              
!!$              !----------------------------------------------
...
!!$              !----------------------------------------------
!!$
!!$!-------- This array were already allocated in the normal part --              
!!$!              ALLOCATE(v_diff(ent))
!!$!              ALLOCATE(v_normal(ent))
!!$              
!!$              !----------------------------------------------
...
!!$                 !-------------------------------------------
!!$                 
!!$                 this%v(1:dim,1:num,1) = v_lub_normal(1:dim,1:num)
!!$                 
!!$		 !** Next subroutine is in colloid_compute_interaction_implicit_velocity_pair_sweep.F90 **
!!$                 CALL colloid_compute_interaction_implicit_velocity_pair_sweep_tang(&
!!$                      this, dt, num_sweep1, stat_info_sub)
...
!!$                    !----------------------------------------
!!$                    
!!$                    this%implicit_pair_sweep_error_tang = error_decrease
!!$                    
!!$                 END IF ! error_decrease < tolerance
!!$                 
!!$              END DO ! num_sweep1 > 0 AND error_decrease < tolerance
...
!!$                 !-------------------------------------------
!!$                 
!!$                 this%implicit_pair_num_sweep_tang = num_sweep_tang     
!!$
!!$              ELSE
!!$                 
!!$                 !-------------------------------------------
...
!!$          
!!$                 this%implicit_pair_num_sweep_tang   = num_sweep_tang                 
!!$                 this%implicit_pair_sweep_error_tang = error_increase
!!$                 
!!$                 !-------------------------------------------
!!$                 ! If not increase, velocity has to be restored.
!!$                 !-------------------------------------------

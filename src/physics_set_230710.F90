...
!************ End of added by Adolfo *******
      ...
      SUBROUTINE physics_set_mem_function(this, Nsteps_memory, E, tau, alpha, beta, dt, stat_info)
        ...
        ENDDO
        ! WRITE(*,*) 'mem function and parameters'
        ! WRITE(*,*) 'E     = ',E
        ! WRITE(*,*) 'Beta  = ',beta
        ! WRITE(*,*) 'Mem   = ',this%mem_function(I)
        ! WRITE(*,*) 'I     = ',I
        ! WRITE(*,*) 'Nstep = ',Nsteps_memory
        ! WRITE(*,*) 'dt    = ',Delta_t
...
SUBROUTINE mittag_leffler(E, x, alpha, beta, kmin, kmax, tol, error_out)
  ...
  !    E_{alpha,beta}(x) = sum_{k=0}^\infinity x^k / Gamma(alpha k + beta)
  ...
  k = 0
  DO WHILE(control)
     E = E + x**k / gamma(alpha*REAL(k, KIND = MK) + beta)
     ...
     IF (k > kmax) THEN
        error_out = -1
        WRITE(*,*) '*** Special function Mittag-Leffler error: convergence '

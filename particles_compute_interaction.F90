SUBROUTINE particles_compute_interaction(this,stat_info)
  ...
  !               newtonian case. (Adolfo Vazquez-Quesada)
  !
...
SUBROUTINE particles_compute_gradx_prev(this,stat_info)
  ...
  !----------------------------------------------------
  !
  ! Purpose     : Computing the gradient of previous positions.
  !
  !----------------------------------------------------
  ...
  IF(ASSOCIATED(this%gradx_prev)) THEN
     DEALLOCATE(this%gradx_prev)
  END IF

  IF(  symmetry ) THEN

     ALLOCATE(this%gradx_prev(num_dim**2*Npoints_integration,this%num_part_all),&
          STAT=stat_info_sub)

  ELSE

     ALLOCATE(this%gradx_prev(num_dim**2*Npoints_integration,this%num_part_real),&
          STAT=stat_info_sub)
              ...
              DO iinter = 1, nnp
                 ...
                 DO ipart = istart, iend
                    ...
                    DO jpart = jstart, jend
                       ...
                       !**** This part is from pp_interaction.inc ****
                       !----------------------------------------------------------
                       ...
                       !-- Information in Npoints_integration must be used because it is the most recent data we have. --
                       DO T = 1, Npoints_integration
                          ...
                          DO b = 1, num_dim  ! ---, row direction
                             ...
                             DO a = 1, num_dim ! |,  column direction
                                ...
                                aux = (x_jp_prev - x_ip_prev) * &
                                     rij(a) * gradw / dij

                                this%gradx_prev((n-1)*Npoints_integration+T, ip) = &
                                     this%gradx_prev((n-1)*Npoints_integration+T, ip) + &
                                     aux / this%rho(ip)

                                IF ( symmetry ) THEN
                                   this%gradx_prev((n-1)*Npoints_integration+T, jp) = &
                                        this%gradx_prev((n-1)*Npoints_integration+T, jp) + &
                                        aux / this%rho(jp)
                                ENDIF
                             ENDDO
